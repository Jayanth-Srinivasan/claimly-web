...
export async function handleUpdateIntakeState(
  ...
  try {
    ...
    await upsertIntakeState({
      ...
      user_id: user.id,
      current_stage: updates.current_stage || 'initial_contact',
      coverage_type_ids: updates.coverage_type_ids || null,
      incident_description: updates.incident_description || null,
      database_questions_asked: updates.database_questions_asked || null,
      validation_passed: updates.validation_passed || null,
      validation_errors: updates.validation_errors || null,
    })

    return {
      success: true,
      data: { message: 'Intake state updated' },
    }
  } catch (error) {
    return {
      success: false,
...
export async function handleCreateClaim(
  ...
  try {
    ...
    // Update chat session to link claim
    await supabase
      .from('chat_sessions')
